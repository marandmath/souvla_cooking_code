"""
Plot distribution of heat in zeroth order approximation and error via the snapshots of simple_model_robin.py.
...
def main(ω, capture_dt):
    ...
    abs_error_loaded = np.load('abs_error.npz')
    abs_error_list = [abs_error_loaded[f'arr_{i}.npy'] for i in range(len(abs_error_loaded.files))]
    ...
    λ = R_m/R_f # d'less
    R = λ/(1-λ)
    rho = np.linspace(0, R_m, 72) # Nρ/4 since Chebyshev non-periodic basis excludes the nodes
    ...
    quad2 = axs[1].pcolormesh(θ, ρ, abs_error_list[0], cmap=plt.cm.seismic, shading='gouraud')
    fig.colorbar(quad2, ax=axs[1])
    axs[1].grid()
    axs[1].set_title(r"$Absolute Error(ρ,θ)$")
    ...
    def animate(i):
        ...
        quad2 = axs[1].pcolormesh(θ, ρ, abs_error_list[i], cmap=plt.cm.seismic, shading='gouraud')
        fig.suptitle(f"t = {i*capture_dt:.3f}s")
        # quad1.set_array(T_ε_list[i].ravel())
        # quad2.set_array(error_list[i].ravel())
        axs[0].arrow(0, 0, ω*i*capture_dt,  R_m, length_includes_head=True)
        axs[1].arrow(0, 0, ω*i*capture_dt,  R_m, length_includes_head=True)
        return quad1, quad2, 
